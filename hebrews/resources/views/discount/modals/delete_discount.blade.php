<div
    ...
    tabindex="-1"
    aria-labelledby="deleteDiscountModalTitle"
    aria-modal="true"
    role="dialog"
    >
    <div class="relative w-auto pointer-events-none modal-dialog modal-dialog-centered">
        ...
        <div class="relative p-4 modal-body">
            <form id="delete-discount-form" action="{{ route('discount.delete') }}" method="post">
                @csrf
                <label class="block mb-4 text-sm">
                    <p>Are you sure you want to delete this item (name: <span x-text="$store.discount.deleteData?.name"></span>)?</p>
                    <input name="id" type="hidden" :value="$store.discount.deleteData?.id">
                </label>
            </form>
        </div>
        <div
