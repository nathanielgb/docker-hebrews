<div
    ...
    id="deleteMenuModal"
    tabindex="-1"
    aria-labelledby="deleteMenuModalTitle"
    aria-modal="true"
    role="dialog"
    >
    <div class="relative w-auto pointer-events-none modal-dialog modal-dialog-centered">
        ...
        <div class="relative p-4 modal-body">
            <form id="delete-menu-form" action="{{ route('menu.delete') }}" method="post">
                @csrf
                <label class="block mb-4 text-sm">
                    <p>Are you sure you want to delete this menu item (name: <span x-text="$store.menu.deleteMenuData?.name"></span>)?</p>
                    <input name="id" type="hidden" :value="$store.menu.deleteMenuData?.id">
                </label>
            </form>
        </div>
        <div
