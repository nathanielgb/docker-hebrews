<div
    ...
    tabindex="-1"
    aria-labelledby="deleteInventoryModalTitle"
    aria-modal="true"
    role="dialog"
    >
    <div class="relative w-auto pointer-events-none modal-dialog modal-dialog-centered">
        ...
        <div class="relative p-4 modal-body">
            <form id="delete-inventory-form" action="{{ route('menu.delete_inventory') }}" method="post">
                @csrf
                <label class="block mb-4 text-sm">
                    <p>Are you sure you want to delete this inventory item (name: <span x-text="$store.inventory.deleteInventoryData?.name" class="font-bold"></span>)?</p>
                    <input name="id" type="hidden" :value="$store.inventory.deleteInventoryData?.id">
                </label>
            </form>
        </div>
        <div
