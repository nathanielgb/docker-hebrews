<div
    ...
    id="deleteCartModal"
    tabindex="-1"
    aria-labelledby="deleteCartModalTitle"
    aria-modal="true"
    role="dialog"
    >
    <div class="relative w-auto pointer-events-none modal-dialog modal-dialog-centered">
        ...
        <div class="relative p-4 modal-body">
            <form id="delete-cart-form" action="{{ route('order.delete_cart') }}" method="post" autocomplete="off">
                @csrf
                <label class="block mb-4 text-sm">
                    <p>Are you sure you want to delete this cart item?</p>
                    <input name="id" type="hidden" :value="$store.cart.deleteData.id">
                </label>
            </form>
        </div>
        <div
