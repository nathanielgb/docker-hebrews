<div
    ...
    <div class="relative w-auto pointer-events-none modal-dialog modal-dialog-centered">
        ...
        <div class="relative p-4 modal-body">
            <form id="delete-item-form" action="{{ route('order.delete_item') }}" method="post" autocomplete="off">
                @csrf
                <label class="block mb-4 text-sm">
                    <p>Are you sure you want to delete this order item (name: <span x-text="$store.item.delete.name" class="font-bold"></span>)?</p>
                    <input name="id" type="hidden" :value="$store.item.delete.id">
                </label>
            </form>
        </div>
        <div
