<div
    ...
    id="zeroOutAccountModal"
    tabindex="-1"
    aria-labelledby="zeroOutAccountModalTitle"
    aria-modal="true"
    role="dialog"
    >
    <div class="relative w-auto pointer-events-none modal-dialog modal-dialog-centered">
        ...
        <div class="relative p-4 modal-body">
            <form id="zeroout-account-form" action="{{ route('bank.account.reset') }}" method="post">
                @csrf
                <label class="block mb-4 text-sm">
                    <p>Are you sure you want to  <span class="font-bold">reset</span> your running balance to <span class="font-bold">ZERO</span>? Account (name: <span x-text="$store.transactions.data?.account_name" class="font-bold"></span>). </p>
                    <input name="id" type="hidden" :value="$store.transactions.data?.id">
                </label>
            </form>
        </div>
        <div
